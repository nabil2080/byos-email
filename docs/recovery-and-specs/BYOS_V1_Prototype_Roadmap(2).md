...
- Next.js + React + TypeScript + Tailwind is the control plane/public management site. 
...
|Frontend|Node.js + TypeScript|Web applications|
|Mailbox UI|SolidJS + Tailwind|Interactive webmail|
|Controlplane|Next.js + React + Tailwind|Public/control/admin experience|
...
### **Control Plane / Management Website** 
...
/ /pricing /security /privacy /docs /blog /login /signup /dashboard /organization /domains /users /billing /storage /security 

**BOUNDARY:** The control plane is not the mailbox data processor. It manages business state and permissions while the mailbox client handles encrypted mailbox interaction. 
...
- Vite / SolidStart where client/server rendering is useful 
...
|Message actions|Compose, reply, replyall, forward, signatures|
|Data|Attachments, inline images, contacts, aliases|
|Automation|Filters/rules, forwarding, auto-reply, scheduled send|
|Privacy|Local decryption, local search/index, securitysettings|



#### **Rendering model** 

Public/control pages -> SSR/SSG where appropriate Mailbox -> interactive client application + local crypto + local index 
...
Every paid plan receives the same complete core BYOS email product. Customers pay primarily for mailbox capacity and scale, not for artificial feature unlocks. 

|**Plan**|**Mailboxes**|**Monthly**|**Annual**|
|---|---|---|---|
|Solo|1|$3|$31|
|Starter|5|$14|$143|
|Business|10|$26|$265|
|Team|25|$60|$612|
|Business+|50|$110|$1,122|
|Enterprise|50+|Custom|Custom|
...
**PRICING:** S3 is not a premium feature. A developer can use Solo and connect their own S3-compatible storage. 
...
Control Plane: Next.js + React + Tailwind Mail Client: SolidJS + TypeScript + Tailwind Crypto: Rust/WASM shared client module 
...
<!-- Start of picture text -->
                              INTERNET<br>                                 |<br>                           +-----v------+<br>                           | CLOUDFLARE |<br>                           +-----+------+<br>                                 |<br>                 +---------------+----------------+<br>                 |                                |<br>          +------v-------+                  +-----v------+<br>          | CONTROL      |                  | WEBMAIL    |<br>          | PLANE        |                  | CLIENT     |<br>          | Next.js      |                  | SolidJS    |<br>          | React        |                  | TypeScript |<br>          +------+-------+                  +-----+------+<br>                 |                                |<br>                 +---------------+----------------+<br>                                 |<br>                              HTTPS/API<br>                                 |<br>                           +-----v------+<br>                           | GO PLATFORM|<br>                           +-----+------+<br>                                 |<br>              +------------------+------------------+<br>              |                                     |<br>        +-----v------+                         +----v-----+<br>        | MAIL       |                         | DATA      |<br>        | Postfix    |                         | Postgres  |<br>        | Rspamd     |                         | Redis     |<br>        | Queue      |                         +----------+<br>        +-----+------+<br>              |<br>              v<br>      Customer-controlled storage<br>      Google Drive / S3 / S3-compatible<br><!-- End of picture text -->
...
|Frontend split|Next.js/React controlplane + SolidJS mailbox.|
...
The V1 product is a complete business-email service with normal internet interoperability, customercontrolled persistent storage, privacy-aware mailbox encryption, organization administration, recovery, scheduled sending, and a path to standard desktop clients. The architecture is intentionally split so each major concern has a clear home: Next.js/React for the control plane, SolidJS for the mailbox, Rust/WASM for client cryptography, and Go/Postfix/Rspamd for the mail platform. 
